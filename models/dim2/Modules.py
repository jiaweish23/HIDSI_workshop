...
class ResnetBlock1d(nn.Module):
    ...
    def __init__(self, dim, *, time_emb_dim=None, groups=8):
        super().__init__()
        self.mlp = (
        ...
        self.block1 = Block1d(dim, groups=groups)
        self.block2 = Block1d(dim, groups=groups)
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim, heads=4, dim_head=32, dropout=0.):
        super().__init__()
...
class Unet_SR(nn.Module):
    def __init__(
        ...
        init_dim = default(init_dim, dim)
        self.init_conv = nn.Conv2d(input_channels+1, init_dim, 1, padding=0)  # changed to 1 and 0 from 7,3
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
                        Residual(PreNorm(dim_out, LinearAttention(dim_out))),
                        #Residual(PreNorm(dim_out, CrossAttention(dim_out, context_dim))),
