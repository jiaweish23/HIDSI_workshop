...
@torch.no_grad()
def p_sample_ddim_loop(model, shape, sigma_t=0., Guided=False,
                  c=None, unconditional_guidance_scale=1., unconditional_conditioning=None, img=None):
    device = next(model.parameters()).device
    ...
    for i in tqdm(reversed(range(0, timesteps)), desc='sampling loop time step', total=timesteps):
        img = p_sample_ddim(model, img, torch.full((b,), i, device=device, dtype=torch.long), i, sigma_t, Guided,
                  c, unconditional_guidance_scale, unconditional_conditioning)
        imgs.append(img.cpu().numpy())
    return imgs
...
def sample_ddim(model, image_size, batch_size=16, channels=1):
    return p_sample_ddim_loop(model, shape=(batch_size, channels, image_size, image_size))
